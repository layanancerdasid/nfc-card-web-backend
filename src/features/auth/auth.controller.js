...
const { payloadLoginWrong, emailNotVerified } = require("../../helper/message_response");
...
const loginMember = async (req, res) => {
    try {
        ...
        if (user.email_verified === null) return responseJSON(res, 400, emailNotVerified);

        const isVerifiedPwd = await bcrypt.compareSync(payload.password, user.password);
